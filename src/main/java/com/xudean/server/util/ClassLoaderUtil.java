        ...
        try {
            ...
            while (dirs.hasMoreElements()) {
                ...
                if ("file".equals(protocol)) {
                    ...
                    // 如果是jar包文件
                    // 定义一个JarFile
                    logger.debug("jar.type.scan");
                    try {

                        String filePath = url.getFile().split("!")[0];

                        //针对Tomcate Windows加载jar包兼容
                        if (filePath.toLowerCase().startsWith("file:/") && System.getProperty("os.name").toLowerCase().startsWith("win")) {
